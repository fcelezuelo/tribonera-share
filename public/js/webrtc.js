...
window.TriboneraWebRTC = (function () {
  ...
  // =========================================================================
  // VOICE CHANNEL MESH SUBSYSTEM (Discord-Style Real-time Audio Calls)
  // =========================================================================
  ...
  // Map of peerSocketId => RTCPeerConnection (Voice Mesh)
  const voicePeerConnections = new Map();
  // Map of peerSocketId => HTMLAudioElement
  const voiceAudioElements = new Map();
  // Map of peerSocketId => RTCIceCandidate[] queue
  const voiceIceQueues = new Map();

  function initVoiceSocket(s) {
    voiceSocket = s;
  }
  ...
  async function startLocalMic() {
    ...
    try {
      ...
      setupVoiceActivityDetection(localMicStream);
      return localMicStream;
    } catch (err) {
      console.warn('[VoiceMesh] Microfone não acessível ou permissão negada. Entrando como ouvinte:', err);
      return null;
    }
  }
  ...
  function setupVoiceActivityDetection(stream) {
    try {
      ...
      vadInterval = setInterval(() => {
        if (isMicMuted || !currentVoiceChannelId || !voiceSocket) {
          if (isSpeaking) {
            isSpeaking = false;
            voiceSocket.emit('voice:speaking', { isSpeaking: false });
            if (window.TriboneraApp?.onSelfSpeaking) window.TriboneraApp.onSelfSpeaking(false);
          }
          return;
        }
        ...
        // VAD threshold
        if (avg > 18) {
          speakingCounter = 5; // keep active for at least ~250ms
          if (!isSpeaking) {
            isSpeaking = true;
            voiceSocket.emit('voice:speaking', { isSpeaking: true });
            if (window.TriboneraApp?.onSelfSpeaking) window.TriboneraApp.onSelfSpeaking(true);
          }
        } else {
      ...
      console.warn('[Voice VAD Init Error]:', e);
    }
  }
  ...
  function createVoicePeerConnection(targetSocketId) {
    ...
    voiceIceQueues.set(targetSocketId, []);
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate && voiceSocket) {
        voiceSocket.emit('voice:peer-ice-candidate', {
          targetSocketId,
          candidate: event.candidate
        });
      }
    ...
    // Incoming Remote Audio Track from Peer
    pc.ontrack = (event) => {
      let audioEl = voiceAudioElements.get(targetSocketId);
      if (!audioEl) {
        ...
        voiceAudioElements.set(targetSocketId, audioEl);
      }
  ...
  function cleanupVoicePeer(targetSocketId) {
    if (voicePeerConnections.has(targetSocketId)) {
      const pc = voicePeerConnections.get(targetSocketId);
      pc.close();
      voicePeerConnections.delete(targetSocketId);
    }
    if (voiceAudioElements.has(targetSocketId)) {
      const el = voiceAudioElements.get(targetSocketId);
      el.pause();
      el.srcObject = null;
      el.remove();
      voiceAudioElements.delete(targetSocketId);
    }
    voiceIceQueues.delete(targetSocketId);
  }

  async function joinVoiceChannel(channelId, channelName, existingPeers = []) {
    currentVoiceChannelId = channelId;
    await startLocalMic();

    if (window.TriboneraSound) {
      window.TriboneraSound.play('voiceJoin');
    }

    // Connect to each peer already in the channel
    for (const peer of existingPeers) {
      if (peer.socketId === voiceSocket?.id) continue;
      const pc = createVoicePeerConnection(peer.socketId);
      try {
        const offer = await pc.createOffer({
        ...
        voiceSocket.emit('voice:peer-offer', {
          targetSocketId: peer.socketId,
          offer: pc.localDescription
        });
      } catch (err) {
        console.error('[VoiceMesh CreateOffer Error]:', err);
      }
    }
  }

  function leaveVoiceChannel() {
    if (window.TriboneraSound && currentVoiceChannelId) {
      window.TriboneraSound.play('voiceLeave');
    }

    currentVoiceChannelId = null;

    // Close all voice peer connections
    for (const [targetSocketId] of voicePeerConnections.entries()) {
      cleanupVoicePeer(targetSocketId);
    }
  ...
  async function handleVoiceUserJoined({ socketId }) {
    // When another user joins, they will send an offer to us, or we create a peer connection
    if (!voicePeerConnections.has(socketId)) {
      createVoicePeerConnection(socketId);
    }
  }
  ...
  async function handleVoicePeerOffer({ fromSocketId, offer }) {
    let pc = voicePeerConnections.get(fromSocketId);
    if (!pc) {
      pc = createVoicePeerConnection(fromSocketId);
    }
    ...
    try {
      ...
      // Flush ICE queue
      const queue = voiceIceQueues.get(fromSocketId) || [];
      while (queue.length > 0) {
        const cand = queue.shift();
      ...
      if (voiceSocket) {
        voiceSocket.emit('voice:peer-answer', {
          targetSocketId: fromSocketId,
          answer: pc.localDescription
        });
      }
    } catch (err) {
      console.error('[Voice handlePeerOffer Error]:', err);
    }
  }

  async function handleVoicePeerAnswer({ fromSocketId, answer }) {
    const pc = voicePeerConnections.get(fromSocketId);
    if (!pc) return;
    ...
    try {
      ...
      console.error('[Voice handlePeerAnswer Error]:', err);
    }
  }

  async function handleVoicePeerIceCandidate({ fromSocketId, candidate }) {
    const pc = voicePeerConnections.get(fromSocketId);
    if (!pc || !pc.remoteDescription) {
      const queue = voiceIceQueues.get(fromSocketId) || [];
      queue.push(candidate);
      voiceIceQueues.set(fromSocketId, queue);
      return;
    }
  ...
  function setMicMuted(muted) {
    ...
    if (voiceSocket && currentVoiceChannelId) {
      voiceSocket.emit('voice:state-change', {
        isMuted: isMicMuted,
  ...
  function setDeafened(deaf) {
    ...
    for (const [, audioEl] of voiceAudioElements.entries()) {
      audioEl.muted = isDeafened;
    }
    if (voiceSocket && currentVoiceChannelId) {
      voiceSocket.emit('voice:state-change', {
  ...
  return {
    ...
    handleVoiceUserLeft,
    handleVoicePeerOffer,
    handleVoicePeerAnswer,
    handleVoicePeerIceCandidate,
    setMicMuted,
    setDeafened,
    isMicMuted: () => isMicMuted,
    isDeafened: () => isDeafened,
    getCurrentVoiceChannelId: () => currentVoiceChannelId
  };
})();
